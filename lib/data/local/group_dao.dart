...
class GroupDao extends DatabaseAccessor<AppDatabase> with _$GroupDaoMixin {
  ...
  Stream<List<Group>> watchGroupsForUser(int userId) {
    ...
    final query = select(groups).join([
      ...
      ..where(
        (groups.isDeleted.equals(false) | groups.isDeleted.isNull()) &
            (groups.ownerUserId.equals(userId) |
                membershipsForUser.id.isNotNull()),
      )
      ..orderBy([
    ...
    return query.watch().map(
          (rows) =>
              rows.map((row) => row.readTable(groups)).toList(growable: false),
        );
  }
  ...
  Future<List<Group>> getGroupsForUser(int userId) {
    final membershipsForUser =
        alias(groupMembers, 'memberships_for_user_fetch');
    final query = select(groups).join([
      leftOuterJoin(
        membershipsForUser,
        membershipsForUser.groupId.equalsExp(groups.id) &
  ...
  Future<Group?> findGroupByRemoteId(String remoteId) {
    return (select(groups)..where((tbl) => tbl.remoteId.equals(remoteId)))
        .getSingleOrNull();
  }
  ...
  Future<int> updateGroupFields(
      {required int groupId, required GroupsCompanion entry}) {
    return (update(groups)..where((tbl) => tbl.id.equals(groupId)))
        .write(entry);
  }

  Future<void> softDeleteGroup(
      {required int groupId, required DateTime timestamp}) {
  ...
  Stream<List<GroupMemberDetail>> watchMemberDetails(int groupId) {
    final query = select(groupMembers).join([
      leftOuterJoin(
          localUsers, localUsers.id.equalsExp(groupMembers.memberUserId)),
    ])
      ..where(groupMembers.groupId.equals(groupId) &
          (groupMembers.isDeleted.equals(false) |
              groupMembers.isDeleted.isNull()));
  ...
  Future<GroupMember?> findMemberIncludingDeleted(
      {required int groupId, required int userId}) {
    return (select(groupMembers)
          ..where((tbl) =>
              tbl.groupId.equals(groupId) & tbl.memberUserId.equals(userId)))
        .getSingleOrNull();
  }

  Future<GroupMember?> findMemberByRemoteId(String remoteId) {
    return (select(groupMembers)..where((tbl) => tbl.remoteId.equals(remoteId)))
        .getSingleOrNull();
  }

  Future<int> insertMember(GroupMembersCompanion entry) =>
      into(groupMembers).insert(entry);

  Future<int> updateMember(
      {required int memberId, required GroupMembersCompanion entry}) {
    return (update(groupMembers)..where((tbl) => tbl.id.equals(memberId)))
        .write(entry);
  }
  ...
  Future<void> softDeleteMember(
      {required int memberId, required DateTime timestamp}) {
    return (update(groupMembers)..where((tbl) => tbl.id.equals(memberId)))
        .write(
  ...
  Future<SharedBook?> findSharedBookById(int id) {
    return (select(sharedBooks)..where((tbl) => tbl.id.equals(id)))
        .getSingleOrNull();
  }

  Future<SharedBook?> findSharedBookByRemoteId(String remoteId) {
    return (select(sharedBooks)..where((tbl) => tbl.remoteId.equals(remoteId)))
        .getSingleOrNull();
  }

  Future<int> insertSharedBook(SharedBooksCompanion entry) =>
      into(sharedBooks).insert(entry);
  ...
  Future<List<SharedBook>> findSharedBooksByGroupId(int groupId) {
    return (select(sharedBooks)..where((tbl) => tbl.groupId.equals(groupId)))
        .get();
  }

  Future<List<SharedBook>> getDirtySharedBooks() {
    return (select(sharedBooks)..where((tbl) => tbl.isDirty.equals(true)))
        .get();
  }
  ...
  Future<List<SharedBookDetail>> fetchSharedBooksPage({
    ...
    if (excludeIsbns != null && excludeIsbns.isNotEmpty) {
      query.where(books.isbn.isNotIn(excludeIsbns));
    }
    ...
    if (trimmedQuery != null && trimmedQuery.isNotEmpty) {
      final pattern =
          '%${trimmedQuery.replaceAll('%', '').replaceAll('_', '')}%';
      query.where(
  ...
  Future<int> updateSharedBook(
      {required int sharedBookId, required SharedBooksCompanion entry}) {
    return (update(sharedBooks)..where((tbl) => tbl.id.equals(sharedBookId)))
        .write(entry);
  }

  Future<int> updateSharedBookFields(
      {required int sharedBookId, required SharedBooksCompanion entry}) {
    return (update(sharedBooks)..where((tbl) => tbl.id.equals(sharedBookId)))
        .write(entry);
  }

  Future<void> softDeleteSharedBook(
      {required int sharedBookId, required DateTime timestamp}) {
    return (update(sharedBooks)..where((tbl) => tbl.id.equals(sharedBookId)))
        .write(
  ...
  Future<void> deleteSharedBook(int sharedBookId) {
    return (delete(sharedBooks)..where((tbl) => tbl.id.equals(sharedBookId)))
        .go();
  }
  ...
  Stream<List<GroupInvitationDetail>> watchInvitationDetailsForGroup(
      int groupId) {
    ...
    final joined = select(groupInvitations).join([
      leftOuterJoin(inviterAlias,
          inviterAlias.id.equalsExp(groupInvitations.inviterUserId)),
      leftOuterJoin(
        acceptedAlias,
      ...
      ..where(groupInvitations.groupId.equals(groupId) &
          ...
          (groupInvitations.isDeleted.equals(false) |
              groupInvitations.isDeleted.isNull()));
  ...
  Future<int> updateInvitation(
      {required int invitationId, required GroupInvitationsCompanion entry}) {
    return (update(groupInvitations)
          ..where((tbl) => tbl.id.equals(invitationId)))
        .write(entry);
  }

  Future<void> softDeleteInvitation(
      {required int invitationId, required DateTime timestamp}) {
    return (update(groupInvitations)
          ..where((tbl) => tbl.id.equals(invitationId)))
        .write(
  ...
  Future<void> deleteInvitation({required int invitationId}) {
    return (delete(groupInvitations)
          ..where((tbl) => tbl.id.equals(invitationId)))
        .go();
  }
  ...
  Stream<List<Loan>> watchLoansForGroup(int groupId) {
    ...
    return query
        .watch()
        .map((rows) => rows.map((row) => row.readTable(loans)).toList());
  }
  ...
  Stream<List<LoanDetail>> watchLoanDetailsForGroup(int groupId) {
    ...
    final query = select(loans).join([
      ...
      leftOuterJoin(
          borrowers,
          borrowers.id.equalsExp(loans.borrowerUserId) |
              loans.borrowerUserId.isNull()),
    ...
    return query.watch().map(
      (rows) {
        ...
        if (kDebugMode) {
          debugPrint(
              '[DAO DEBUG] watchLoanDetailsForGroup($groupId) found ${loanDetails.length} unique loans');
          for (final detail in loanDetails) {
            debugPrint(
                '[DAO DEBUG] Loan: ${detail.loan.uuid}, status: ${detail.loan.status}, borrowerUserId: ${detail.loan.borrowerUserId}, externalName: ${detail.loan.externalBorrowerName}');
          }
        }
  ...
  Future<List<LoanDetail>> getAllLoanDetails() {
    ...
    final query = select(loans).join([
      leftOuterJoin(sharedBooks, sharedBooks.id.equalsExp(loans.sharedBookId)),
      // Join book either via sharedBook or directly via loans.bookId
      leftOuterJoin(
          books,
          (sharedBooks.id.isNotNull() &
                  books.id.equalsExp(sharedBooks.bookId)) |
              (sharedBooks.id.isNull() & books.id.equalsExp(loans.bookId))),
  ...
  Future<List<LoanDetail>> getAllLoanDetailsForUser(int userId) {
    ...
    final query = select(loans).join([
      ...
      ..where((loans.isDeleted.equals(false) | loans.isDeleted.isNull()) &
          (loans.lenderUserId.equals(userId) |
              loans.borrowerUserId.equals(userId)));
  ...
  Stream<List<LoanDetail>> watchAllLoanDetailsForUser(int userId) {
    final borrowers = alias(localUsers, 'allBorrowersStream');
    final owners = alias(localUsers, 'allOwnersStream');
    final query = select(loans).join([
      leftOuterJoin(sharedBooks, sharedBooks.id.equalsExp(loans.sharedBookId)),
      leftOuterJoin(
          books,
  ...
  Future<Loan?> findLoanByRemoteId(String remoteId) {
    return (select(loans)..where((tbl) => tbl.remoteId.equals(remoteId)))
        .getSingleOrNull();
  }
  ...
  Future<Loan?> findLoanByUuid(String uuid) {
    return (select(loans)..where((tbl) => tbl.uuid.equals(uuid)))
        .getSingleOrNull();
  }

  Future<int> updateLoanFields(
      {required int loanId, required LoansCompanion entry}) {
    return (update(loans)..where((tbl) => tbl.id.equals(loanId))).write(entry);
  }

  Future<int> updateLoanStatus(
      {required int loanId, required LoansCompanion entry}) {
    return (update(loans)..where((tbl) => tbl.id.equals(loanId))).write(entry);
  }

  Future<void> softDeleteLoan(
      {required int loanId, required DateTime timestamp}) {
  ...
  Future<int> deleteOldLoans(DateTime cutoffDate) {
    return (delete(loans)
          ..where((tbl) =>
              (tbl.status.equals('rejected') | tbl.status.equals('cancelled')) &
              tbl.updatedAt.isSmallerThanValue(cutoffDate)))
        .go();
  }
}
